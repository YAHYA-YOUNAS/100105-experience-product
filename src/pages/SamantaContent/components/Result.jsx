...
function Result({ result, email, title, content }) {
  ...
  return (
    <div className="w-11/12 p-3 mx-auto border border-gray-300 shadow-lg">
      <h2 className="text-center pb-2 font-bold text-xl border-b border-gray-200">Result From Samanta</h2>
      ...
      {success ? 
        <p className="p-2 rounded text-center bg-neutral-100 text-green-500 w-fit font-semibold mx-auto mt-2">{success}</p> 
        : 
        <p className="text-red-500 text-lg font-bold text-center mt-2">{error}</p>}
    </div>
  );
}
