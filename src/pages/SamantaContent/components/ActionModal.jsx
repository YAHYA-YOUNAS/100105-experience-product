...
function ActionModal({
  ...
  const handleCheck = async () => {
    ...
    try {
      setLoading(true);
      const response = await axios.post("https://100085.pythonanywhere.com/uxlivinglab/v1/content-scan/4f0bd662-8456-4b2e-afa6-293d4135facf/",
        {
          title,
          content,
        }
  ...
  const handleContribute = (e) => {
    ...
    window.open("https://dowellpay.online/contribute-payment/?product_number=UXLIVINGLAB001", '_blank');
  };
        ...
        <div className="relative w-11/12 my-6 mx-auto max-w-3xl">
          <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none max-h-screen overflow-y-auto">
            <div className="mb-3">
              ...
              {showTimesUsed && (
                <p className="p-2 rounded text-center bg-neutral-100 text-black w-fit mx-auto mt-4">
                  You have used Samanta Content Evaluator {numofTimes} times
                </p>
              )}
              <div className="buttons flex gap-4 justify-center mt-4">
                ...
                {contribute && (
                  <button className="bg-green-500 hover:bg-green-400 text-white p-2 text-sm md:text-[15px] rounded-md flex gap-2 items-center" 
              ...
              {(contribute && !result) && (
                <div className="flex gap-2 mt-10 justify-center items-center border-t pt-3">
                  <span className="text-sm md:text-base">Do you have a coupon?</span>
                  <button
                ...
                <div className="scale mt-5">
                  ...
                  <div className="scaleButtons px-3 flex flex-wrap gap-2 w-fit mx-auto">
                    {scaleData.map((item, index) => (
                      <button
                        key={index}
                        className="px-2.5 text-white bg-green-500 rounded-lg"
              ...
              {result && 
                <button
                  className="flex items-center gap-2 text-white bg-slate-700 hover:bg-slate-500 rounded-md p-2 my-5 mx-auto"
              ...
              {globalState.couponSuccess && (
                <p
                  className={
                    showSuccess ? "text-green-500 font-semibold w-fit mx-auto py-3" : "hidden"
                  }
                >
                  {globalState.couponSuccess}
                </p>
