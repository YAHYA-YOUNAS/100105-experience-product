...
function Form({ passProps }) {
        ...
        if (!email || !firstSelection || !secondSelection) {
            ...
            if (response.success) {
                ...
                passProps({
                    ...
                    firstSelection,
                    secondSelection,
                    occurrences: emailInfodata.occurrences,
                })
            } else {
    ...
    return (
        <form className="flex flex-col gap-3 pb-2">
            <Selection
                title="Select First License"
                selectedValue={firstSelection}
                data={data}
                onSelectChange={(value) => setFirstSelection(value)}
            />
            <Selection
                title="Select Second License"
                selectedValue={secondSelection}
                data={data}
                onSelectChange={(value) => setSecondSelection(value)}
            />
            <Input
                type="email"
                inputValue={email}
            ...
            {validationError && (
                <Message
                    classes="text-red-500 font-bold w-50 mx-auto px-4"
                    message={validationError}
                />
            )}

            <div className="flex flex-wrap justify-center md:flex-row md:gap-4 mx-auto">
                <Button
                    type="button"
                ...
                <Button
                    type="button"
                    classes="bg-yellow-500 px-2 py-1"
                    name="Reset"
                    onButtonClick={handleResetClick}
                />
                <Button
                    type="button"
                    classes="bg-green-700 px-2 py-1"
                    name="Experience"
                    loading={loading}
                    showIcon="experience"
                    onButtonClick={handleExperienceClick}
                />
            </div>
        </form>
    )
